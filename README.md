...
1. 浏览器 `localStorage` 中的临时本地数据
2. 当前页面同级目录下的配置文件，例如 `./data.json`
3. 空列表

如果远程数据加载失败，页面会使用同级目录下的本地文件兜底。例如数据路径是 `viewing-records/json.data`，页面会优先尝试同级文件名 `./json.data`，再尝试 `./viewing-records/json.data`。
...
如果 GitHub 保存仍然失败，编辑页会提供三个兜底操作：

- 重试 GitHub
- 保存到当前浏览器本地数据
- 复制完整 JSON，手动提交到数据仓库

选择“保存到本地”后，系统会自动切换到“本地数据”模式，避免返回列表时又读取远程旧数据。
