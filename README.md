...
- `提交数据`: 把当前本地缓存中的完整数据提交到 GitHub 远程文件

如果你在设置里把数据路径配置成 `viewing-records/json.data`，本地兜底读取也会优先尝试同级文件名 `./json.data`，再尝试 `./viewing-records/json.data`。
...
页面会自动从数据文件地址中识别 GitHub 用户名、仓库名、分支和文件路径。需要手动调整时，再展开“高级设置”。
