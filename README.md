...
一个部署在 GitHub Pages 上的 Vue 3 看房记录工具。页面负责展示、对比、编辑房源；数据默认保存在浏览器本地缓存中，远程同步通过 GitHub 仓库里的 JSON 文件手动重置、同步和提交。
...
| 页面 | 地址 | 说明 |
|---|---|---|
| 展示页 | `/` 或 `/index.html` | 房源卡片/列表展示、排序、对比、删除、数据设置 |
...
data.json               # 示例数据/手动备份，可复制到远程数据文件
```
...
构建后会生成 `dist/`，并写入 `.nojekyll` 供 GitHub Pages 发布。数据文件不再随页面构建产物发布。
...
当前页面以浏览器本地缓存为主，远程 GitHub 文件只在手动操作时参与。

页面加载、详情页、编辑页默认读取：

1. 浏览器 `localStorage` 中的本地缓存
2. 空列表

编辑、新增、删除只会更新本地缓存，不会自动提交远程。

展示页数据按钮说明：

- `重置`: 清空本地缓存里的房源数据，重新拉取 GitHub 远程数据并覆盖本地缓存
- `同步`: 拉取 GitHub 远程数据并合并到本地缓存；同一房源以本地缓存为准，远程只补充本地没有的房源
- `提交`: 先拉取 GitHub 最新提交信息，再把当前本地缓存提交到 GitHub 远程文件
...
- 数据仓库：私有仓库，只保存远程数据文件，例如 `viewing-records/json.data`

在展示页点击 `设置`，日常只需要填写：

| 字段 | 示例 |
|---|---|
| 数据文件地址 | `https://github.com/yatxuan/jsonData/blob/main/viewing-records/json.data` |
| Personal Access Token | `github_pat_...` 或 `ghp_...` |

页面会自动从数据文件地址中识别 GitHub 用户名、仓库名、分支和文件路径。

私有仓库必须配置 Token，否则浏览器访问 GitHub API 会返回 404 或 401，重置、同步和提交远程都会失败。

设置弹窗里的“测试连接”会用当前配置读取一次数据，成功时显示读取到的条数。

设置弹窗里的“复制当前 JSON”会复制页面当前本地缓存里的数据，方便手动备份或粘贴回数据仓库。
...
页面不会区分 Token 类型，请求 GitHub API 时都会使用 `Authorization: Bearer <token>`。

Token 只保存在当前浏览器的 `localStorage`，不会提交到仓库。不要把 Token 写进 `data.json`、HTML 或 README。

## 数据字段

远程数据文件是一个 JSON 数组，每一项是一套房源。当前字段以新版编辑页为准，不再兼容旧的单个 `wechat` 字段。
...
- `sharedKitchenLocation`: 公用厨房位置，可选“和阳台共用”“门口玄关共用”
...
- 仓库名、分支、文件路径填错
- 数据文件地址不是 GitHub 文件页面、Raw 地址或 GitHub Contents API 地址
...
409 通常表示远程文件已变化，本地提交时带的 `sha` 旧了。当前代码会重新拉取最新 `sha` 后再提交一次。
