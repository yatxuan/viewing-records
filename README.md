...
一个部署在 GitHub Pages 上的 Vue 3 看房记录工具。页面负责展示、筛选、对比、编辑房源；数据默认保存在浏览器本地缓存中，远程同步通过 GitHub 仓库里的 JSON 文件手动刷新和提交。
...
- `刷新数据`: 从 GitHub 拉取远程数据，并合并到本地缓存；同一房源以本地缓存为准，远程只补充本地没有的房源
- `提交数据`: 先拉取 GitHub 最新数据，与当前本地缓存合并后，再提交到 GitHub 远程文件
...
私有仓库必须配置 Token，否则浏览器访问 GitHub API 会返回 404 或 401，刷新远程和提交远程都会失败。
